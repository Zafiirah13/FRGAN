...
class DCGAN(object):
  def __init__(self, sess, input_height=108, input_width=108, is_crop=True,
    ...
    """
    #print('ext, name',input_fname_pattern,dataset_name)
    self.sess = sess
  ...
  def train(self, config):
    ...
    if config.dataset == 'mnist':
      ...
      #print('data path: ',"./data", config.dataset, self.input_fname_pattern,data)
    ...
    if config.dataset == 'mnist':
      ...
      if (self.is_grayscale):
        sample_inputs = np.array(sample).astype(np.float32)[:,:,:, None]#[:, :, :, None] Modified
      else:
  ...
  def load(self, checkpoint_dir,subdir=None):
    if subdir is None: subdir=self.model_dir
